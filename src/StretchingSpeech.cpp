...
//  The purpose of this program is to control the panda robot via ros and speech
...
void chatterSpeech(const std_msgs::String::ConstPtr& msg){
    ROS_INFO("I heard: [%s]", msg->data.c_str());
    ...
    std::cout << global_order_movement << "\n";
    std::cout << global_order_pos << "\n";
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub = node_handle.subscribe("stt_node", 1000, chatterSpeech);
    ros::Publisher pub = node_handle.advertise<std_msgs::String>("tts_node", 1000);
    ...
    try{
        ...
        while(ros::ok()) {
            ...
            // Defaul Value for Language, because I still haven't figured out how I can change that one...
            //language = "de";

            // publish state
            //if language == "en":
			//    global_response = "espeak -ven+m1 'Task was executed successfully'"
		    //else:
			//    global_response = "espeak -vde+m1 'Aufgabe wurde erfolgreich ausgeführt'"
            // This is the Response from the robot if it is moving or not, necessary for the opcua master to wait until "stopped"
            // so that it could run the conveyor belt
            global_response = "espeak -vde+m1 'Aufgabe wurde erfolgreich ausgeführt'";
            global_ros_response.data = global_response;
